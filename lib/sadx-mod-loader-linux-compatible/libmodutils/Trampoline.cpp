	...
	, revert(destructRevert)
{
	if (start > end)
	{
		throw std::runtime_error("Start address cannot exceed end address.");
	}

	if (end - start < 5)
	{
		throw std::runtime_error("Length cannot be less than 5 bytes.");
	}
	...
	if (codeData == nullptr)
	{
		throw std::runtime_error("VirtualAlloc failure.");
	}
