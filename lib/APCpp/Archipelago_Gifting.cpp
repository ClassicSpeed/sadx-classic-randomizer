...
AP_RequestStatus AP_SetGiftBoxProperties(AP_GiftBoxProperties props)
{
    ...
    // Set Values
    AP_CommitServerData();
    while (req_local_box.status == AP_RequestStatus::Pending && req_global_box.status == AP_RequestStatus::Pending &&
...
void handleGiftAPISetReply(const AP_SetReply& reply)
{
    if (reply.key == AP_PLAYER_GIFTBOX_KEY)
    {
        ...
        for (std::string gift_id : local_giftbox.getMemberNames())
        {
            ...
            gift.ItemValue = local_giftbox[gift_id].get("item_value", 0).asUInt64();
            //technically this value is unbounded so even uint64 isnt enough
        ...
        if (gifting_autoReject)
        {
            ...
            {
                std::set<std::string> desired_traits(local_box_props.DesiredTraits.begin(),
                                                     local_box_props.DesiredTraits.end());
                for (const std::pair<std::string, AP_Gift>& gift : gifts)
                {
                    bool found = false;
    ...
    else if (reply.key.rfind("GiftBoxes;", 0) == 0)
    {
        ...
        for (std::string motherbox_slot : json_data.getMemberNames())
        {
            ...
            int min_supported_version = player_global_props.get("minimum_gift_data_version",
                                                                CURRENT_GIFT_PROTOCOL_VERSION).asInt();
            int max_supported_version = player_global_props.get("maximum_gift_data_version",
                                                                CURRENT_GIFT_PROTOCOL_VERSION).asInt();

            if (max_supported_version < CURRENT_GIFT_PROTOCOL_VERSION || min_supported_version >
                CURRENT_GIFT_PROTOCOL_VERSION)
