...
bool parse_response(std::string msg, std::string& request)
{
    ...
    for (unsigned int i = 0; i < root.size(); i++)
    {
        ...
        else if (cmd == "Connected")
        {
            ...
            // MUST be called before resetitemvalues, otherwise PrivateServerDataPrefix, GetPlayerID return broken values!
            ...
            if (gifting_supported)
            {
                ...
                AP_SetNotify("GiftBox;" + std::to_string(ap_player_team) + ";" + std::to_string(ap_player_id),
                             AP_DataType::Raw, true);
            }

            if ((root[i]["slot_data"].get("death_link", false).asBool() || root[i]["slot_data"].get("DeathLink", false).
                asBool()) && deathlinksupported) enable_deathlink = true;
            ...
            for (auto slot_itr = root[i]["slot_data"].begin(); slot_itr != root[i]["slot_data"].end(); ++slot_itr)
            {
                ...
                {
                    if (key != "death_link" && key != "death_link_amnesty" && key != "DeathLink" && key !=
                        "DeathLink_Amnesty")
        ...
        else if (cmd == "PrintJSON")
        {
            ...
            {
                if (getPlayer(0, root[i]["receiving"].asInt()).alias == getPlayer(0, ap_player_id).alias ||
                    getPlayer(0, root[i]["item"]["player"].asInt()).alias != getPlayer(0, ap_player_id).alias) continue;
            ...
            else if (printType == "Hint")
            {
                ...
                msg->text = std::string("Item ") + msg->item + std::string(" from ") + msg->sendPlayer +
                    std::string(" to ") + msg->recvPlayer + std::string(" at ") + msg->location + std::string(
                        (msg->checked ? " (Checked)" : " (Unchecked)"));
                messageQueue.push_back(msg);
            }
            ...
            if (!bouncedfunc)
            {
                ...
                for (unsigned int j = 0; j < root[i]["tags"].size(); j++)
                {
                    if (root[i]["tags"][j].asString() == "DeathLink")
                    {
                        ...
                        if (recvdeath)
                        {
                            std::string cause = root[i]["data"]["cause"].isNull()
                                                    ? ""
...
std::string getItemName(std::string game, int64_t id)
{
    ...
    return map_item_id_name.count(item)
               ? map_item_id_name.at(item)
               : std::string("Unknown Item") + std::to_string(id) + " from " + game;
}

std::string getLocationName(std::string game, int64_t id)
{
    std::pair<std::string, int64_t> location = {game, id};
    return map_location_id_name.count(location)
               ? map_location_id_name.at(location)
               : std::string("Unknown Location") + std::to_string(id) + " from " + game;
}
