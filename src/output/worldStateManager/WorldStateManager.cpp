...
WorldStateManager::WorldStateManager(Options& options, Settings& settings, GameStatus& gameStatus): _options(options),
    _settings(settings), _gameStatus(gameStatus), _doorIndicatorManager(DoorIndicatorManager::Init()),
...
BOOL WorldStateManager::OnIsWindyValleyOpen()
{
    ...
    if (CurrentCharacter == Characters_Gamma)
        return EventFlagArray[FLAG_E102_MR_WINDYSTONE] && _instance->_levelEntrances.canEnter(
            WindyValley, CurrentCharacter);
    return _instance->_levelEntrances.canEnter(WindyValley, CurrentCharacter) && _instance->
